...
class Kindaid_Footer_Newsletter_Widget extends WP_Widget
{
    public function __construct()
    {
        parent::__construct(
            ...
            ['description' => __('A widget to display footer logo, info text, newsletter form, and social links.', 'kindaid')]
        );
    ...
    public function widget($args, $instance)
    {
        ...
        $newsletter_shortcod = !empty($instance['newsletter_shortcod']) ? $instance['newsletter_shortcod'] : '';
        ...
        // Footer Info
        if ($footer_info): ?>
            <p class="tp-footer-dec mb-15"><?php echo esc_html($footer_info); ?></p>
        <?php endif;
        ...
        <div class="tp-footer-subscribe p-relative mb-30">
            <?php
            if (!empty($newsletter_shortcod)): ?>
                <?php echo do_shortcode($newsletter_shortcod); ?>
    ...
    public function form($instance)
    {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('newsletter_shortcod'); ?>">Newsletter Shortcode:</label>
            <input class="widefat" id="<?php echo $this->get_field_id('newsletter_shortcod'); ?>"
                name="<?php echo $this->get_field_name('newsletter_shortcod'); ?>" type="text"
                value="<?php echo esc_attr($instance['newsletter_shortcod']); ?>" placeholder="[newsletter_form]" />
        </p>
                ...
                $(document).ready(function () {
                    $('body').on('click', '.upload-logo-button', function (e) {
                        ...
                        file_frame.on('select', function () {
                            ...
                            file_frame.close();
                            $('.media-modal, .media-modal-backdrop').remove();
                        });
                        file_frame.open();
                    });
                });
    ...
    public function update($new_instance, $old_instance)
    {
        ...
        $instance['footer_info'] = sanitize_textarea_field($new_instance['footer_info']);
        $instance['newsletter_shortcod'] = sanitize_text_field($new_instance['newsletter_shortcod']);
